...
b.alamat , a.swni , a.email_pemohon , a. telp_pemohon , b.no_rt , b.no_rw , a.no_form
...
if(isset($_POST['nskp'])) {  
  ...
  $sql="UPDATE `datang_header` SET `SWNI` = '$nskp', `NIK_PEMOHON` = '$nikpemohon', `EMAIL_PEMOHON` = '$email' , `TELP_PEMOHON` = '$hp' , `CREATED_AT` = '$tgl'   WHERE  `datang_header`.`NIK_PEMOHON` = '$nik';";
  $id = mysqli_query($con,$sql);
  $zz="UPDATE `datang_detail` SET `NIK_PEMOHON` = '$nikpemohon' WHERE  `datang_detail`.`NIK_PEMOHON` = '$nik';";
...
if ($id) {
  ...
  function genNoForm($con, $date) {
    $gen=mysqli_fetch_array(mysqli_query($con,"SELECT no_form from datang_header where NO_FORM like 'REGS/D/3578/$date/%' ORDER BY id DESC LIMIT 1  "));
    // $no_datang = 0;
    if (empty($gen[0])) {
     $no_datang = "REGS/D/3578/".$date."/0000";
    } else {
      $no_datang = $gen[0];
    }
 ...
 return "REGS/D/3578/$date/$maxno_datang";
}
  if(isset($_POST['nskp'])) {  
  $nskp = $_POST['nskp'];
  $noform = $_POST['noform'];
  ...
  $hp = $_POST['hp'];
  $tgl = date('Y-m-d H:i:s');
  $ketemu=mysqli_num_rows(mysqli_query($con,"SELECT id from datang_header where NIK_PEMOHON =  '$nikpemohon' "));
  if (empty($ketemu)) {
    $sql = "insert into datang_header (NO_FORM,SWNI,NIK_PEMOHON , EMAIL_PEMOHON,  TELP_PEMOHON , CREATED_AT ,IS_DELETE , FLAG_STATUS) values ('$noform','$nskp','$nikpemohon','$email','$hp','$tgl','1','0')";
  $id = mysqli_query($con,$sql);
  $kepkel=mysqli_fetch_array(mysqli_query($con,"SELECT id from datang_header where nik_pemohon='$nikpemohon' "));
  $shdk = '8';
  $id_datang = $kepkel['id'];
  $tgl = date('Y-m-d H:i:s');
  $qq="insert into datang_detail (ID_DATANG,NIK_PEMOHON,SHDK,CREATED_AT) values ('$id_datang','$nikpemohon','$shdk','$tgl')";
            ...
            <form class="form-horizontal" method="post" enctype="multipart/form-data">
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <input type="text" class="form-control"  placeholder="Nomor Surat Keterangan Pindah" name="nskp" value="<?php if (isset($_GET['nik'])){ echo $c['swni'];}?>" required>
                  </div>
                </div>
                <div class="form-group">
