<?php
    ...
    $id = $_POST['id'];
    mysqli_query($con,"update keluar_header set flag_status = '$app' , tgl_app = '$tgl' where id='$id'");
    $c=mysqli_fetch_array(mysqli_query($con,"SELECT nik_pemohon,email_pemohon,(SELECT nama_lgkp FROM biodata_wni WHERE biodata_wni.`NIK`=keluar_header.`NIK_PEMOHON`) AS nama_lgkp FROM keluar_header WHERE id = '$id' "));
    $nik_pemohon = $c['nik_pemohon'];
    $nama = $c['nama_lgkp'];

    // email
    $subjek="PENDAFTARAN PERMOHONAN PINDAH KELUAR DISPENDUKCAPIL";
    $body             = 
